	...
	_makeBuffer() {
		...
		for (var xx = -10; xx <= 10; xx++) {
			for (var yy = -10; yy <= 0; yy++) {
				...
				let rad = (11 + yy) * 30 + randomFloat(-20, 20);
		...
		for (var ii = 0; ii < delaunay.coords.length / 2; ii++) {
			...
			else coords[3 * ii + 2] = randomFloat(0, 2); // - 100 * Math.sin(delaunay.coords[2 * ii + 1] / 400 * 5);
		...
		for (let ii = 0; ii < indices.length; ii += 3) {
			if (
				...
				for (let kk = 0; kk < 3; kk++) {
					...
					let colorRate = mix(1.0, 0.65, mixRate);
